...
class CGNS_VAL_USER_Checks(CGNS.VAL.parse.generic.GenericParser):
  ...
  def Zone_t(self,pth,node,parent,tree,log):
    ...
    if 1:
      ...
      if (CGU.getShape(node)!=shp):
        rs=log.push(pth,'S192',CGU.getShape(node),shp,node[0])
        bad_value = True
      elif (isStructured):
        zd=node[1]
        for nd in range(id):
        ...
        if ((zd[nd][0]<=zd[nd][1]) or (zd[nd][2]!=0)):
          rs=log.push(pth,'S010')
          bad_value = True       
        if (not CGU.hasChildType(node,CGK.Elements_ts)): # check if has Elements_t nodes
          rs=log.push(pth,'S605')       
      ...
      if (    not bad_value 
        ...
        # Checking correct combination of element ranges using of a table initialized at 0 and 
        # incremented by 1 for each element found in ElementRange children 
        cellsize = self.context[CGK.CellSize_s][pth][0] # number of cells in zone
  #      tcell = NPY.zeros(cellsize,dtype='i4', order='F') # table of zeros of size CellSize
        tall  = NPY.zeros(cellsize,dtype='i4', order='F')
        has_nface = False # initialize
        has_ngon  = False # initialize
        facesize  = 0     # initialize
        erl = dict(zip(range(len(CGK.ElementType)),NPY.empty([len(CGK.ElementType),0]).tolist())) # initialize element range list
        ...
        for erpath in erpathlist: # loop on element range paths
          ...
          print 'ZAZA',erpath,CGU.getPathFromNode(tree,node)
          erp = CGU.getParentFromNode(tree,er) # element_t node
          et  = erp[1][0]                      # element type (integer)
          shp = (id,2)
          if (CGU.getShape(er) == shp) and (CGU.getValueDataType(er) in [CGK.I4, CGK.I8]): # checking shape and type
            ...
            if (all(er[1][0] > 0)): 
              tall[range(min(er[1][0])-1,max(er[1][0]))] += 1 # add 1 to table 
  #            if (et in val_u.getElementTypes(CGK.Cell_s,self.context[CGK.PhysicalDimension_s][pth])): # consider only cell Elements in tcell
  #              tcell[range(min(er[1][0])-1,min(max(er[1][0]),cellsize))] += 1 # add 1 to table
              erl[et].append([min(er[1][0]),max(er[1][0])]) # add to element range list, reordered min,max
          if   (et == CGK.NFACE_n): has_nface = True                  
          elif (et == CGK.NGON_n):  has_ngon  = True            
        if ( erpathlist and # (any(tcell != 1) or ## # not in SIDS  
             any(tall != 1)): rs=log.push(pth,'S209') # raise error if any element of table not equal to 1
        if (has_nface and not has_ngon): rs=log.push(pth,'S198',CGK.NGON_n_s,CGK.NFACE_n_s)
        self.context[CGK.ElementsSize_s][pth]=len(tall) # total number of elements in zone
        self.context[CGK.ElementRangeList_s][pth]=erl   # range indices for each element type
      elif (    not bad_value 
            and isStructured):
        if (CGU.hasChildType(node,CGK.Elements_ts)): # Structured zone should not have Elements_t
          rs=log.push(pth,'S608')
        # check all boundary faces are either on a BC or a GridConnectivity node
        # Only for structured zones here, test for unstructured zones is performed in Elements_t
        (rl,al) = val_u.getIndicesOnBCandGC(tree,pth,id) # get IndexRange and IndexArray on BC and GridConnectivity
        bndfaces = val_u.initBndFaces(zd) # create boundary face tables, full of 0, face-centered
        for r in rl:
          (bnd,imin,imax,jmin,jmax)=val_u.getFaceNumber(r,zd) # get face number and indices, vertex-centered
          if (bnd in range(cd*2)):
            bndfaces[bnd][imin-1:imax-1,jmin-1:jmax-1] += 1 # add 1 to faces in range r         
        for bnd in range(cd*2): # loop on boundary faces
          if (any(bndfaces[bnd].flatten() == 0)): # not defined faces
            rs=log.push(pth,'S702',val_u.bdnName(bnd),NPY.where(bndfaces[bnd] == 0))              
          if (any(bndfaces[bnd].flatten() > 1)): # doubly defined faces
            rs=log.push(pth,'S703',val_u.bdnName(bnd),NPY.where(bndfaces[bnd] > 1))   
  ...
  def Family_t(self,pth,node,parent,tree,log):
    ...
    for fpth in CGU.getAllParentTypePaths(CGK.FamilyName_ts):
      ...
      if (found): break
    if (not found):
      rs=log.push(pth,'S303')
    return rs
  # --------------------------------------------------------------------
  def IndexArray_t(self,pth,node,parent,tree,log):
    rs=CGM.CHECK_OK
    if ((CGU.getAuthNames(node) is not None) 
        and (node[0] not in CGU.getAuthNames(node))):  # checking name
      rs=log.push(pth,'S122',node[0],node[3],CGU.getAuthNames(node)) 
    # Getting referred zone (current or donor)
    zpth = None
    if (parent[3] in CGU.getAuthParentTypes(node)):
      if (node[0] in [CGK.PointList_s,CGK.InwardNormalList_s]):
        zpth = pth
            ...
            and parent[3] in [CGK.GridConnectivity1to1_ts,CGK.GridConnectivity_ts]):
    ...
    if (zpth is not None):
      ...
      if (    (CGU.getAuthDataTypes(node) is not None) 
        ...
        rs=log.push(pth,'S199',ntype,CGU.getAuthDataTypes(node))
      else :
        if (node[0] == CGK.InwardNormalList_s):
          if (ntype not in [CGK.R4, CGK.R8]): # checking data type
            rs=log.push(pth,'S199',ntype,[CGK.R4, CGK.R8])
          ...
          if (listlength > 0):
            ...
            if (shp != targetshp): # checking shape
              rs=log.push(pth,'S192',shp,targetshp,node[0])
          else:
            if (   len(shp) != 2
                or shp[0]   != self.context[CGK.PhysicalDimension_s][zpth]): # checking shape
              rs=log.push(pth,'S192',shp,'(%s,?)' %self.context[CGK.PhysicalDimension_s][zpth],node[0])            
        else: # node[0] != CGK.InwardNormalList_s
          if (CGU.getShape(node)[0]!=idim):  # checking shape
            rs=log.push(pth,'S192',CGU.getShape(node),idim,node[0])          
          if (ntype not in [CGK.I4,CGK.I8]): # checking data type 
            rs=log.push(pth,'S199',ntype,[CGK.I4,CGK.I8])
          else:
            pl=node[1]
            if (pl.size!=len(set(pl.flatten().tolist()))): # could use also NPY.unique()
              rs=log.push(pth,'S208') # checking each value appears only once in list
            if (any(pl.flatten()<1)): rs=log.push(pth,'S206') # checking out of range (<1) 
            if (parent[3] == CGK.BCDataSet_ts):
              refnode = CGU.getNodeByPath(tree,CGU.getPathAncestor(pth,level=2)) # BC_t
            else:
              refnode = parent
            (gridloc,rs)  = val_u.getGridLocation(refnode,pth=pth,log=log,rs=rs)
            if (    gridloc != CGK.CellCenter_s 
                and node[0] == CGK.CellListDonor_s): # cell data required for cell list donor
              rs=log.push(pth,'S218',gridloc,node[0],[CGK.CellCenter_s])
            elif (    (gridloc==CGK.Vertex_s # Vertex data 
                       or
              ...
              for d in range(idim):
                if (any(pl[d]>dd[d])):
                  rs=log.push(pth,'S211',CGK.Vertex_s,d+1,idim,[1,dd[d]]) # checking out of range elements
              ...
              for d in range(idim):
                if (any(pl[d]>dd[d])):
                  rs=log.push(pth,'S211',CGK.CellCenter_s,d+1,idim,[1,dd[d]]) # checking out of range elements
            else:
...
#                if (gridloc==CGK.FaceCenter_s): imin=self.context[CGK.CellSize_s][zpth][0]+1  # not in SIDS                   
                ...
                if (any(pl[0]<imin) or any(pl[0]>imax)):
                  rs=log.push(pth,'S211',gridloc,1,idim,[imin,imax]) # checking out of range elements
        ...
        if (CGU.getValueDataType(parent) in [CGK.C1]):
          ...
          if (zdonor is not None): zpth = targetpath
            ...
            and parent[3] not in [CGK.GridConnectivity1to1_ts,
                                  CGK.GridConnectivity_ts]):
        rs=log.push(pth,'S005',node[0],node[3])
      else:
        zpth = pth
    if (zpth is not None):          
      ...
      ntype = CGU.getValueDataType(node)
      ztype = val_u.getZoneType(tree,zpth)
      if (CGU.getShape(node)!=shp): # checking shape
        rs=log.push(pth,'S192',CGU.getShape(node),shp,node[0])
      elif (    (CGU.getAuthDataTypes(node) is not None) 
            and (ntype not in CGU.getAuthDataTypes(node))):
        # checking data type 
        rs=log.push(pth,'S199',ntype,CGU.getAuthDataTypes(node))
      else:
        pr=node[1]
        if (any(pr.flatten()<1)): rs=log.push(pth,'S206')
        # checking out of range (<1) 
        for d in range(self.context[CGK.IndexDimension_s][zpth]):
          if (pr[d][0]>pr[d][1]): rs=log.push(pth,'S207')
        ...
        if (node[0] in [CGK.PointRange_s,CGK.PointRangeDonor_s]):
          ...
          if ( (gridloc==CGK.Vertex_s # Vertex data  
            ...
            if (    ztype==CGK.Structured_s
              ...
              if (not val_u.isAFace(pr)): rs=log.push(pth,'S219')
          elif (    (gridloc==CGK.CellCenter_s) # Cell center data 
                and (ztype==CGK.Structured_s) # for structured zone 
            ...
            for d in range(self.context[CGK.IndexDimension_s][zpth]):
              if (any(pr[d]>dd[d])):
                rs=log.push(pth,'S211',CGK.CellCenter_s,d+1,self.context[CGK.IndexDimension_s][zpth],[1,dd[d]]) # checking out of range elements
...
#              if (gridloc==CGK.FaceCenter_s): imin=self.context[CGK.CellSize_s][zpth][0]+1 # not in SIDS 
              ...
              if (any(pr.flatten()<imin) or any(pr.flatten()>imax)):
                rs=log.push(pth,'S211',gridloc,1,self.context[CGK.IndexDimension_s][zpth],[imin,imax]) # checking out of range elements
              erl = val_u.getElementTypeRangeList(val_u.getLevelFromGridLoc(gridloc),
                                                  self.context[CGK.PhysicalDimension_s][zpth],
                                                  self.context[CGK.ElementRangeList_s][zpth])
          ...
          if (ztype!=CGK.Unstructured_s or parent[3] != CGK.Elements_ts):
            rs=log.push(pth,'S609') # name reserved for elements of unstructured zones
          else: 
            dd=self.context[CGK.ElementsSize_s][zpth]
            if (any(pr.flatten()>dd)):
              rs=log.push(pth,'S217',1,self.context[CGK.IndexDimension_s][zpth],[1,dd]) # checking out of range elements
    return rs
  # --------------------------------------------------------------------
  def BC_t(self,pth,node,parent,tree,log):
    ...
    if (gridloc not in agridloc):# BC applies only on surface patch
      rs=log.push(pth,'S218',gridloc,node[3],agridloc) 
    return rs
  # --------------------------------------------------------------------
    ...
    if (datasize):
      for n in CGU.hasChildType(node,CGK.DataArray_ts): # check DataArray children shapes
        if (CGU.getShape(n)!=datasize): 
          rs=log.push(pth,'S192',CGU.getShape(n),datasize,n[0])          
    return rs
  # --------------------------------------------------------------------
  def ZoneSubRegion_t(self,pth,node,parent,tree,log):
    rs=CGM.CHECK_OK
    if (CGU.getShape(node)!=(1,)):  # check shape
      rs=log.push(pth,'S192',CGU.getShape(node),(1,),node[0])
    ...
    if (gridloc not in val_u.getAuthGridLocation(celldim,regioncelldim)): # check valid grid location
      rs=log.push(pth,'S215',gridloc,celldim,regioncelldim) 
    ...
    if (CGU.hasChildName(node,CGK.BCRegionName_s)): # test if BC exists
      ...
      for path in CGU.getAllNodesByTypeSet(parent,CGK.BC_ts):
        ...
        if (CGU.stringValueMatches(loc,n[0])): 
          ...
          break
      if (not test):
        rs=log.push(pth+'/'+CGK.BCRegionName_s,'S010')
    if (CGU.hasChildName(node,CGK.GridConnectivityRegionName_s)): # test if GridConnectivity exists
      ...
      for path in CGU.getAllNodesByTypeSet(parent,[CGK.GridConnectivity_ts,CGK.GridConnectivity1to1_ts]):
        ...
        if (CGU.stringValueMatches(loc,n[0])): 
          ...
          break
      if (not test):
        rs=log.push(pth+'/'+CGK.GridConnectivityRegionName_s,'S010')
    if (listlength>0):
      for n in CGU.hasChildType(node,CGK.DataArray_ts): # check DataArray children shapes
        shp=(listlength,)
        if (CGU.getShape(n)!=shp): 
          rs=log.push(pth,'S192',CGU.getShape(n),shp,n[0])
    return rs
  # --------------------------------------------------------------------
  ...
  def FamilyName_t(self,pth,node,parent,tree,log):
    ...
    if (famdefinition[1] is None):
      ...
      if (not self.context.has_key(famtargetpath)):
        famtargetnode=CGU.getNodeByPath(tree,famtargetpath)
        if (famtargetnode is None):
          rs=log.push(pth,'S301',famtarget)
        else:
          self.context[famtargetpath][pth]=True
    return rs
  # --------------------------------------------------------------------
  ...
  def GridConnectivity1to1_t(self,pth,node,parent,tree,log):
    rs=CGM.CHECK_OK
    if (val_u.getZoneType(tree,pth)!=CGK.Structured_s): # checking zone is structured
      rs=log.push(pth,'S620')
    er =CGU.hasChildName(node,CGK.PointRange_s)
    erd=CGU.hasChildName(node,CGK.PointRangeDonor_s)
    if (not er):
      rs=log.push(pth,'S194',CGK.PointRange_s)   
    if (not erd):
      rs=log.push(pth,'S194',CGK.PointRangeDonor_s)      
    ntype = CGU.getValueDataType(node)
    if ((CGU.getAuthDataTypes(node) is not None) 
      and (ntype not in CGU.getAuthDataTypes(node))): # checking date type 
      rs=log.push(pth,'S199',ntype,CGU.getAuthDataTypes(node))
      ...
      targetpath='/%s/%s'%(basename,zdonorname)
      zdonor=CGU.getNodeByPath(tree,targetpath)
      if (zdonor is None): rs=log.push(pth,'S401',targetpath)
        ...
        for path in CGU.getAllNodesByTypeSet(zdonor,[CGK.GridConnectivity1to1_ts]):
          ...
          if (CGU.stringValueMatches(n,val_u.getZoneName(tree,pth))):
            ...
            if (    der and derd and er and erd
              ...
              break
        if (not found):
          rs=log.push(pth,'S621',zdonorname)
    (gridloc,rs) = val_u.getGridLocation(node,pth=pth,log=log,rs=rs)
    if (gridloc != CGK.Vertex_s): # checking grid location is vertex
      rs=log.push(pth,'S218',gridloc,node[3],[CGK.Vertex_s])
    return rs
  # --------------------------------------------------------------------
  def GridConnectivity_t(self,pth,node,parent,tree,log):
    ...
    if (not val_u.hasOneAndOnlyOneChildAmong(node,[CGK.PointList_s,CGK.PointRange_s])):
      rs=log.push(pth,'S224',[CGK.PointList_s,CGK.PointRange_s])  
    ...
    if (    gridct in [CGK.Abutting_s,CGK.Abutting1to1_s]
      ...
      rs=log.push(pth,'S218',gridloc,gridct,[CGK.Vertex_s,CGK.FaceCenter_s,
                                             CGK.IFaceCenter_s,CGK.JFaceCenter_s,CGK.KFaceCenter_s])
    elif (    gridct      in [CGK.Overset_s]
          and gridloc not in [CGK.Vertex_s,CGK.CellCenter_s]):
      rs=log.push(pth,'S218',gridloc,gridct,[CGK.Vertex_s,CGK.CellCenter_s])
    return rs
  # --------------------------------------------------------------------
  ...
  def BCDataSet_t(self,pth,node,parent,tree,log):
    ...
    agridloc = val_u.getAuthGridLocation(physdim,physdim-1)
    if (gridloc not in agridloc):# BC applies only on surface patch
      rs=log.push(pth,'S218',gridloc,CGK.BCDataSet_ts,agridloc)      
    return rs
  # --------------------------------------------------------------------
  ...
  def DiffusionModel_t(self,pth,node,parent,tree,log):
    ...
    if (node[1] is None):
      ...
      if (    ((cd==1) and (CGU.getShape(node)[0]!=1))
        ...
        rs=log.push(pth,'S281',cd)
      else:
        v=set(node[1].flat)
        if (not v.issubset(set((0,1)))):
              rs=log.push(pth,'S282')
    return rs
  # --------------------------------------------------------------------
  ...
  def DimensionalUnits_t(self,pth,node,parent,tree,log):
    ...
    if (CGU.getShape(node)!=(32,5)):
      ...
      if (tname not in CGK.MassUnits_l):        rs=log.push(pth,'S110')
      tname=d[1].tostring().rstrip()
      if (tname not in CGK.LengthUnits_l):      rs=log.push(pth,'S111')
      tname=d[2].tostring().rstrip()
      if (tname not in CGK.TimeUnits_l):        rs=log.push(pth,'S112')
      tname=d[3].tostring().rstrip()
      if (tname not in CGK.TemperatureUnits_l): rs=log.push(pth,'S113')
      tname=d[4].tostring().rstrip()
      if (tname not in CGK.AngleUnits_l):       rs=log.push(pth,'S114')
    dc=CGU.hasChildName(parent,CGK.DataClass_s)
    if (dc is None): rs=log.push(pth,'S164')
    return rs
  # --------------------------------------------------------------------
  def AdditionalUnits_t(self,pth,node,parent,tree,log):
    ...
    if (CGU.getShape(node)!=(32,3)):
      ...
      d=node[1].T
      tname=d[0].tostring().rstrip()
      if (tname not in CGK.ElectricCurrentUnits_l):   rs=log.push(pth,'S115')
      tname=d[1].tostring().rstrip()
      if (tname not in CGK.SubstanceAmountUnits_l):   rs=log.push(pth,'S116')
      tname=d[2].tostring().rstrip()
      if (tname not in CGK.LuminousIntensityUnits_l): rs=log.push(pth,'S117')
    return rs
  # --------------------------------------------------------------------
  def DimensionalExponents_t(self,pth,node,parent,tree,log):
    ...
    if (CGU.getShape(node)!=(5,)):
      ...
      rs=log.push(pth,'S199',CGU.getValueDataType(node),[CGK.R4, CGK.R8])
    return rs
  # --------------------------------------------------------------------
  ...
  def DataArray_t(self,pth,node,parent,tree,log):
    ...
    if (dtc is None):
      ...
      if (dtc is None):
        rs=log.push(pth,'S150')
      else:
        rs=log.push(pth,'S155')        
    name=node[0]
    if (name not in CGK.cgnsnames):
      rs=log.push(pth,'S121',name)
    ...
    if ((dtc is not None) and (du is None)):
      if (self.context[CGK.DimensionalUnits_s][pth] is not None):
        ...
        rs=log.push(pth,'S153')
    if ((de is not None) and (dtc is None)): rs=log.push(pth,'S156')
    if ((de is not None) and (du is None)):  rs=log.push(pth,'S157')
    if ((dc is not None) and (dtc is None)): rs=log.push(pth,'S158')
    if ((dc is not None) and (du is None)):  rs=log.push(pth,'S159')
    if ((dc is not None) and (de is None)):  rs=log.push(pth,'S160')
    if (dtc is not None):
      if ((de is not None)
           and CGU.stringValueInList(dtc,[CGK.NormalizedByUnknownDimensional_s,
                                          CGK.DimensionlessConstant_s,
                                          CGK.NondimensionalParameter_s])):
        rs=log.push(pth,'S163')
    return rs
  # --------------------------------------------------------------------
  def GridConnectivityType_t(self,pth,node,parent,tree,log):
    rs=CGM.CHECK_OK
    if (not CGU.stringValueInList(node,CGK.GridConnectivityType_l)):
      rs=log.push(pth,'S104')
    return rs
  # --------------------------------------------------------------------
  def GridLocation_t(self,pth,node,parent,tree,log):
    rs=CGM.CHECK_OK
    if (not CGU.stringValueInList(node,CGK.GridLocation_l)):
      rs=log.push(pth,'S103')
    return rs
  # --------------------------------------------------------------------
  def IntIndexDimension_ts(self,pth,node,parent,tree,log):
    ...
    if (node[0]==CGK.Transform_s):
      if (CGU.getShape(node)!=(idim,)):
        ...
        rs=log.push(pth,'S199',CGU.getValueDataType(node),[CGK.I4,CGK.I8])
      else:
        tr=list(node[1].flat)
        if (not CGS.transformCheckValues(tr,self.context[CGK.CellDimension_s][pth])):
          rs=log.push(pth,'S204')
      ...
      if (CGU.getShape(node)!=(idim,)): # checking shape
        ...
        if (   (any(abs(node[1]) > 1))
          ...
          rs=log.push(pth,'S230',node[0],'among [-1,0,+1], only one nonzero element')
  ...
  def Elements_t(self,pth,node,parent,tree,log):
    ...
    def getElementDataSize(etype,erange,econnect,pth,log,rs):
      ...
      if (not CGU.checkArrayInteger(econnectarray)): # check all data in ElementConnectivity are integers (note: even for MIXED, data are integers since ElementType are described through integers)
        rs=log.push(pth+'/'+CGK.ElementConnectivity_s,'S004')
        return rs,-1,[],[]
        ...
        for i in range(er_start,er_end+1):
          if (index<=ecsize-1): # check ElementConnectivity data long enough to reach index
            ...
            if (ecdata[index]<0 or ecdata[index]>l):
              rs=log.push(pth+'/'+CGK.ElementConnectivity_s,'S109')
              return rs,-1,[],[]
            else:
              ndata = etdata[ecdata[index]] # number of data for the current element
          else:
            rs=log.push(pth+'/'+CGK.ElementConnectivity_s,'S191')
            return rs,-1,[],[]       
...
#          element2=NPY.concatenate((element2,econnect[1][index+1:index+ndata+1])) # List of node (NGON) or face (NFACE) indexes
    ...
    def checkParentElements(penode,int et,pth,log,rs,ppnode=None):
      ...
      for i in range(ejrange):
        ...
        if (etp == -1): # the cell attached to the face has not been found in any ElementRange of authorized cells
          rs=log.push(pth+'/'+CGK.ParentElements_s,'S212',[int(i/2),i%2],CGK.ElementType_[et]) # checking not authorized cell type for the current face type
          return rs
          ...
          if (    (etp in CGK.ElementType_tetra and var not in range(1,4+1))
            ...
            rs=log.push(pth+'/'+CGK.ParentElementsPosition_s,'S213',CGK.ElementType_[etp],var,[int(i/2),i%2]) # bad face position for this element type
            return rs
      return rs
    ...
    def checkAllBndFacesOnBCOrGridConnect(bndfaces,method,tree,pth,log,rs):
      ...
      if (CGU.getShape(er)==(self.context[CGK.IndexDimension_s][pth],2)):
        ...
        if (absent):    rs=log.push(pth,'S700',absent,method)
        if (duplicate): rs=log.push(pth,'S701',duplicate,method)
      return rs
    ...
    if (CGU.getShape(node)!=(2,)):
      ...
      if (er is None) : 
        rs=log.push(pth,'S607') # checking if ElementRange node is present;
        bad_er = True
      else:
        if (CGU.getShape(er)==(self.context[CGK.IndexDimension_s][pth],2)):
          bad_er = False
          ...
          bad_er = True
      if (bad_eb): rs=log.push(pth,'S205') # bad ElementSizeBoundary
      ...
      if (ec is None) : rs=log.push(pth,'S606') # checking if ElementConnectivity node is present
        ...
        if ((CGU.getValueDataType(ec) in [CGK.I4,CGK.I8]) 
          ...
          if (et == CGK.MIXED):
            ...
            if (any(element1 > len(CGK.ElementType))): # checking out of range elements
              rs=log.push(pth+'/'+CGK.ElementConnectivity_s,'S206') 
            ...
            if (any(ec[1] > self.context[CGK.VertexSize_s][pth][0])): 
              rs=log.push(pth+'/'+CGK.ElementConnectivity_s,'S206') # checking out of range elements
      ...
      if ((pe is not None) and (not bad_et)): # ParentElements are provided
        if (et not in val_u.getElementTypes(CGK.Face_s,self.context[CGK.PhysicalDimension_s][pth])):
          rs=log.push(pth+'/'+CGK.ParentElements_s,'S210',CGK.ParentElements_s) # child authorized on face Elements only
        if (not bad_er):
          ...
          if (len(self.context[CGK.CellSize_s][pth])==self.context[CGK.IndexDimension_s][pth]): # if CellSize is defined
            ...
            if (   any(pe[1].flatten()>cellsize) 
              ...
              rs=log.push(pth+'/'+CGK.ParentElements_s,'S206') # checking out of range elements
            else:
              maxpe = NPY.array([max(pei) for pei in pe[1]])
              if (maxpe.min() == 0): # has tracked prohibited (0,0) adjacent cell couples
                rs=log.push(pth+'/'+CGK.ParentElements_s,'S220',NPY.where(maxpe == 0)[0]+min(er[1][0]))
              if (not bad_eb and (eb != 0)):
                ...
                if (any(minpe != 0)): # has tracked elements declared as boundary faces regarding ElementSizeBoundary that have no adjacent cell declared 0 in ParentElements
                  rs=log.push(pth+'/'+CGK.ParentElements_s,'S704',NPY.where(minpe != 0)[0]+min(er[1][0]))
      ...
      if ((pp is not None) and (not bad_et)): # ParentElementsPosition are provided
        if (et not in val_u.getElementTypes(CGK.Face_s,self.context[CGK.PhysicalDimension_s][pth])):
          rs=log.push(pth+'/'+CGK.ParentElementsPosition_s,'S210',CGK.ParentElementsPosition_s) # child authorized on face Elements only
        if (not bad_er):
  ...
  def BaseIterativeData_t(self,pth,node,parent,tree,log):
    ...
    if (CGU.getShape(node)!=(1,)):
      ...
      rs=log.push(pth,'S004')
    elif (node[1][0] < 0):
      rs=log.push(pth,'S502',node[1][0])
    else:
    ...
    if (fnb is None and fpt is not None):
      rs=log.push(pth,'S194',CGK.NumberOfFamilies_s)
    if (fnb is not None and fpt is None):
      rs=log.push(pth,'S194',CGK.FamilyPointers_s)
    if (znb is None and zpt is not None):
      rs=log.push(pth,'S194',CGK.NumberOfZones_s)
    if (znb is not None and zpt is None):
      rs=log.push(pth,'S194',CGK.ZonePointers_s)
    ...
    if (None not in [znb, zpt]):
      ...
      if (CGU.getShape(zpt)!=(32,zmax,self.context[CGK.NumberOfSteps_s][pth])):
          rs=log.push(pth,'S195',zpt[0])
      elif (CGU.getValueDataType(zpt) != CGK.C1):
          rs=log.push(pth,'S196',zpt[0])
      else:
        d=zpt[1].T
        for dlist in d:
          for dname in dlist:
      ...
      if (CGU.getShape(fpt)!=(32,fmax,self.context[CGK.NumberOfSteps_s][pth])):
        ...
        for dlist in d:
          for dname in dlist:
            ...
            tnode=CGU.hasChildName(parent,tname)
            if ((tname!=CGK.Null_s) and (tnode is None)):
              rs=log.push(pth,'S501',tname,fpt[0])
    # TODO: check 503
    return rs
  # --------------------------------------------------------------------
  def ZoneIterativeData_t(self,pth,node,parent,tree,log):
    ...
    if (bitnode is None):
      rs=log.push(pth,'S193',CGK.BaseIterativeData_ts,bpath)
    for ptrname in CGK.PointerNames_l:
      ...
      if (ptrnode is not None):
        ...
        for dname in d:
          ...
          tnode=CGU.hasChildName(parent,tname)
          if ((tname!=CGK.Null_s) and (tnode is None)):
            rs=log.push(pth,'S501',tname,ptrname)
    return rs
  # --------------------------------------------------------------------
  def RigidGridMotion_t(self,pth,node,parent,tree,log):
    ...
    if ((node[1] is None)
      ...
      rs=log.push(pth,'S107')
    ol=CGU.hasChildName(node,CGK.OriginLocation_s)
    if (ol is None):
      rs=log.push(pth,'S194',CGK.OriginLocation_s)
    else:

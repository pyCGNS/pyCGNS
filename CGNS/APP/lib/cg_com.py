...
def parseFile(filename,P,C,L):
  ...
  for p in R:
    L.append((filename,p))
    if (C.path): P.append('%s'%(p[3],))
    else: P.append('%s:%s'%(T[3],p[3]))
  for l in LK:
    if (l[0]==''): FH=l[1]
    else: FH="%s/%s"%(l[0],l[1])
    parseFile(FH,P,C,L)
...
def linkErrorAsString(code):
  s=""
  if (code&CGM.S2P_LKOK):         s+='Link ok'
  if (code&CGM.S2P_LKFAIL):       s+='Link Failed:'
  if (code&CGM.S2P_LKBADSYNTAX):  s+='Link bad syntax'
  if (code&CGM.S2P_LKNOFILE):     s+='Linked-to file not found'
  if (code&CGM.S2P_LKFILENOREAD): s+='Linked-to file not readable'
  if (code&CGM.S2P_LKNONODE):     s+='Linked-to node not found in file'
  if (code&CGM.S2P_LKLOOP):       s+='Link loop detected'
  if (code&CGM.S2P_LKIGNORED):    s+='Link ignored'
  return s

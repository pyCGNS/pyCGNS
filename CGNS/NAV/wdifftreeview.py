    ...
    def paint(self, painter, option, index):
        ...
        if (col==NMT.COLUMN_NAME):
          ...
          if ((self._diag is not None) and (pth in self._diag)):
              color=self.gray
              if (self._diag[pth]==DIFF_NX): color=self.gray
              if (self._diag[pth]==DIFF_NA): color=self.green
              if (self._diag[pth]==DIFF_ND): color=self.red
              painter.save()
...
class Q7Diff(Q7Window,Ui_Q7DiffWindow):
    ...
    def diagAnalysis(self,diag):
        ...
        for k in diag:
            ldiag[k]=DIFF_NX
            for d in diag[k]:
                if (d[0]=='NA'): ldiag[d[1]]=DIFF_NA
                if (d[0]=='ND'): ldiag[d[1]]=DIFF_ND
                if (d[0] in ['CT']): ldiag[k]=DIFF_CT
                if (d[0] in ['C3','C1','C2']): ldiag[k]=DIFF_CQ
                if (d[0] in ['C6','C7']): ldiag[k]=DIFF_CV

...
class Q7DiffTreeModel(Q7TreeModel):
    ...
    def data(self, index, role):
        ...
        if (   (role not in [Qt.BackgroundRole])
            or (self._diag is None)
            or (pth not in self._diag)):
            return Q7TreeModel.data(self, index, role)
        if (col==NMT.COLUMN_NAME):
            if (self._diag[pth]==DIFF_NX): return self.grayLight
            if (self._diag[pth]==DIFF_NA): return self.greenLight
            if (self._diag[pth]==DIFF_ND): return self.redLight
            return self.grayLight
        elif (col==NMT.COLUMN_DATATYPE):
            if (self._diag[pth]==DIFF_CQ): return self.orangeLight
        elif (col==NMT.COLUMN_SHAPE):
            if (self._diag[pth]==DIFF_CS): return self.orangeLight
        elif (col==NMT.COLUMN_SIDS):
            if (self._diag[pth]==DIFF_CT): return self.orangeLight
        return Q7TreeModel.data(self, index, role)
...
class Q7DiffItem(Q7TreeItem):
    def __init__(self,fgprint,data,model,tag="",parent=None):
        super(Q7DiffItem).__init__(self,fgprint,data,model,tag,parent)
    def columnCount(self):
        return NMT.COLUMN_LAST+1
    def data(self,column):
        if (self._itemnode==None):     return self._title[column]
        if (column==NMT.COLUMN_NAME):      return self.sidsName()
